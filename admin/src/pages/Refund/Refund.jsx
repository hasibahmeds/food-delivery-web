// status can be change just for one time
...
//   const handleRefundDecision = async (orderId, decision) => {
...
const Refund = () => {
  ...
  const handleRefundDecision = async (orderId, decision) => {
    ...
    const order = cancelledOrders.find((o) => o._id === orderId);
    const wasAlreadyDecided = order && order.refundStatus !== "pending";

    let confirmMessage = `Are you sure you want to ${decision} refund for this order?`;
    if (wasAlreadyDecided) {
      confirmMessage =
        `This order already has a decision (${order.refundStatus.toUpperCase()}).\n\n` +
        `Do you really want to CHANGE it to "${decision}"?`;
    }
    ...
    try {
      const res = await requestWithFallback(
        "patch",
        `/api/order/${orderId}/refund-status`,
        {
          refundStatus: decision,
        },
      );
      ...
      if (res.data?.success) {
        toast.success(
          `Refund ${decision === "approved" ? "approved" : "rejected"}`,
        );
        fetchCancelledOrders(); // refresh list
      } else {
        toast.error(res.data?.message || "Failed to update");
              ...
              <div className="refund-card-body">
                ...
                <div className="refund-decision">
                  <p style={{ marginBottom: "8px", fontWeight: 500 }}>
                    Current decision:{" "}
                    <span className={`refund-status ${order.refundStatus}`}>
                      {order.refundStatus === "pending"
                        ? "PENDING"
                        : order.refundStatus.toUpperCase()}
                    </span>
                  ...
                  <label>
                    <input
                      ...
                      checked={order.refundStatus === "approved"}
                      onChange={() =>
                        handleRefundDecision(order._id, "approved")
                      }
                  ...
                  <label>
                    <input
                      ...
                      checked={order.refundStatus === "rejected"}
                      onChange={() =>
                        handleRefundDecision(order._id, "rejected")
                      }
                      disabled={updatingId === order._id}
                    />
                    Reject Refund (keep in revenue)
                  ...
                  {/* {order.refundStatus !== "pending" && order.refundDecisionAt && (
                    <p style={{ fontSize: "0.9rem", color: "#888", marginTop: "8px" }}>
                      Last updated: {new Date(order.refundDecisionAt).toLocaleString()}
                    </p>
                  )}
                </div> */}

                  {order.refundStatus !== "pending" &&
                    order.refundDecisionAt && (
                      <p
                        style={{
                          fontSize: "0.92rem",
                          color: "#777",
                      ...
                      >
                        Decided:{" "}
                        {new Date(order.refundDecisionAt).toLocaleString()}
                      </p>
                    )}
                </div>
              </div>
