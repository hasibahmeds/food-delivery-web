...
export const sslSuccess = async (req, res) => {
  try {
    ...
    if (!tran_id) {
       ...
       return res.redirect(`${frontend_url}/order-result?status=success&paymentMethod=Online%20Payment`);
    }
    ...
    res.redirect(`${frontend_url}/order-result?status=success&paymentMethod=Online%20Payment`);
  } catch (error) {
